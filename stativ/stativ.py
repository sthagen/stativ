...
BRM_STORE_ROOT = pathlib.Path(
    BRM_ANCHOR, BRM_NAME, "data", "filestore"
)
ENCODING = "utf-8"
...
def main(argv=None):
    ...
    for path in BRM_BACKUP_ROOT.glob("**/*"):
        ...
        if path.name == f"{BRM_NAME}.properties":
            ...
            for line in text_data:
                if line.strip():
                    ...
                    if rec.startswith("<repoKey>"):
                        repo_key = rec.split("<repoKey>", 1)[1].split("</repoKey>")[
                            0
                        ]
                    ...
                    if rec.startswith("<lastUpdated>"):
                        last_update = rec.split("<lastUpdated>", 1)[1].split(
                            "</lastUpdated>"
                        )[0]
                        meta["last_update"] = (
                            dti.datetime.utcfromtimestamp(
                                float(last_update) / 1.0e3
                            )
                            .astimezone()
                            .isoformat()
                        )
                    ...
                    if next_sha1_hash and rec.startswith("<actual>"):
                        sha1_lc_hex = rec.split("<actual>", 1)[1].split(
                            "</actual>"
                        )[0]
                    ...
                    if next_md5_hash and rec.startswith("<actual>"):
                        md5_lc_hex = rec.split("<actual>", 1)[1].split("</actual>")[
                            0
                        ]
                    ...
                    if next_sha256_hash and rec.startswith("<actual>"):
                        sha256_lc_hex = rec.split("<actual>", 1)[1].split(
                            "</actual>"
                        )[0]
            ...
            if bucket_present:
                ...
                meta["bucket_modify"] = (
                    dti.datetime.utcfromtimestamp(b_stat.st_mtime)
                    .astimezone()
                    .isoformat()
                )
    ...
    print(f"SUMMARY: ENTER({entered}), CHANGE({changed}), KEPT({kept}), GONE({removed}) --> REMAIN({entered + changed + kept})")
    return 0
